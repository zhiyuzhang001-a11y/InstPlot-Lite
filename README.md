...
当前公开版本是 **v0.3.2 未签名预览版**。请在 [Releases 页面](https://github.com/zhiyuzhang001-a11y/InstPlot-Lite/releases/tag/v0.3.2)向下找到 **Assets**，根据电脑选择文件。

> [!NOTE]
> v0.3.2 补全拟合方程与系数在主界面、数据导出和再次导入后的持续显示，修复窗口、曲线可见性和导出选择等问题，并为 Windows 加入安全的一键更新。

### Windows 10 / 11（64 位）

下载 `InstPlot-Lite-0.3.2-windows-x64-setup.exe`，然后双击安装。安装器不要求管理员权限，并会创建开始菜单入口；安装时可以选择是否创建桌面快捷方式。
...
- Apple 芯片（M1、M2、M3、M4 等）：下载 `InstPlot-Lite-0.3.2-macos-arm64.dmg`。

从 v0.3.1 起仅维护 Apple Silicon（arm64）版本，不再提供或更新 Intel Mac 安装包。打开 DMG 后，将 **InstPlot Lite** 拖入 **Applications（应用程序）**。

### Linux（64 位）

- Ubuntu / Debian：下载 `instplot-lite_0.3.2_amd64.deb`，用系统的软件安装器打开。
- 其他兼容发行版：下载 `InstPlot-Lite-0.3.2-linux-x86_64.tar.gz`，解压后运行其中的 `instplot-lite`。
