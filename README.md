...
当前公开版本是 **v0.3.7 未签名预览版**。请在 [Releases 页面](https://github.com/zhiyuzhang001-a11y/InstPlot-Lite/releases/tag/v0.3.7)向下找到 **Assets**，根据电脑选择文件。

> [!NOTE]
> v0.3.7 使用安装器的标准安装后启动机制：应用内更新会自动下载、校验、覆盖安装并重新打开 InstPlot Lite。Windows 主程序和安装包均使用正式图标。v0.3.2 的更新器存在 64 字节签名边界错误，必须手动覆盖安装 v0.3.3 或更高版本一次。

### Windows 10 / 11（64 位）

下载 `InstPlot-Lite-0.3.7-windows-x64-setup.exe`，然后双击安装。安装器不要求管理员权限，并会创建开始菜单入口；安装时可以选择是否创建桌面快捷方式。

从 v0.3.3 起，Windows 版本可以可靠检查 OSS 上经过 Ed25519 签名的更新清单。发现新版时点击“更新并重启”即可自动下载、校验、覆盖安装并重新打开程序。v0.3.2 用户必须手动覆盖安装 v0.3.3 或更高版本一次。由于安装程序尚无商业代码签名，Windows 仍可能显示“未知发布者”或 SmartScreen 提示；这与应用自身执行的发布签名和 SHA-256 完整性校验是两套机制。

### macOS

- Apple 芯片（M1、M2、M3、M4 等）：下载 `InstPlot-Lite-0.3.7-macos-arm64.dmg`。

从 v0.3.1 起仅维护 Apple Silicon（arm64）版本，不再提供或更新 Intel Mac 安装包。打开 DMG 后，将 **InstPlot Lite** 拖入 **Applications（应用程序）**。

### Linux（64 位）

- Ubuntu / Debian：下载 `instplot-lite_0.3.7_amd64.deb`，用系统的软件安装器打开。
- 其他兼容发行版：下载 `InstPlot-Lite-0.3.7-linux-x86_64.tar.gz`，解压后运行其中的 `instplot-lite`。
